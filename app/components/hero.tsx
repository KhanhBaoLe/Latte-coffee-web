'use client';
import { mainFeaturedProduct } from '@/app/data/products';
...
export default function Hero() {
  ...
  const discountPercent = mainFeaturedProduct.originalPrice 
    ? Math.round(
        ((mainFeaturedProduct.originalPrice - mainFeaturedProduct.price) / 
        mainFeaturedProduct.originalPrice * 100
      )
    ): 0;

  const handleMainClick = () => router.push(`/detail/${mainFeaturedProduct.id}`);
        ...
        <div className="lg:w-4/5 space-y-8 space-x-20">
          <div className="space-y-4">
            ...
            <h1 className="text-5xl md:text-6xl font-bold text-stone-900 leading-tight">
              {mainFeaturedProduct.title.split(' ')[0]}
              <span className="bg-gradient-to-r from-emerald-600 to-teal-500 bg-clip-text text-transparent">
                {mainFeaturedProduct.title.split(' ').slice(1).join(' ')}
              </span>
            </h1>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex text-amber-400">
              {[...Array(Math.floor(mainFeaturedProduct.rating))].map((_, i) => (
              ...
              {(mainFeaturedProduct.rating % 1 > 0) && (
              ...
              {[...Array(5 - Math.ceil(mainFeaturedProduct.rating))].map((_, i) => (
            ...
            <span className="text-stone-600 font-medium">
              {mainFeaturedProduct.rating}/5 ({mainFeaturedProduct.reviews} reviews)
            </span>
          </div>
          ...
          <div className="flex items-baseline gap-4">
            <span className="text-5xl font-bold bg-gradient-to-r from-emerald-700 to-teal-600 bg-clip-text text-transparent">
              ${mainFeaturedProduct.price.toFixed(2)}
            </span>
            {mainFeaturedProduct.originalPrice && (
              <>
                <span className="text-stone-500 line-through">${mainFeaturedProduct.originalPrice.toFixed(2)}</span>
          ...
          <div className="flex gap-4">
            <Link 
              href={`/detail/${mainFeaturedProduct.id}`}
